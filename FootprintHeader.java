...
public class FootprintHeader extends FootprintElementArchetype
{
  ...
  long kicadPenWidthNm = 150000; //0.15mm
    ...
    if (tokens[0].startsWith("T0"))
      {
        ...
        kicadPenWidthNm = convertToNanometres(Float.parseFloat(tokens[6]), metric);
        // we skip Kicad's unusedFlagN, visible and layer fields as
  ...
  public String lihataText(long xOffset, long yOffset, float magnificationRatio) {
    return "      ha:text." + textCount++ + " {\n       scale = " + (int)(100*magnificationRatio) + 
        "\n       thickness = " + kicadPenWidthNm + "nm\n" +
        "       x = " + (long)((textxCoordNm + xOffsetNm)*magnificationRatio) + "nm\n" +
        "       y = " + (long)((textyCoordNm + yOffsetNm)*magnificationRatio) + "nm\n" +
        "       rot = " + kicadTextRotation/10 +
        "\n       string = " + kicadDeviceDescription +
        "\n       fid = 0\n" +
        "       ha:attributes {\n       }\n       ha:flags {\n        clearline = 1\n       }\n      }\n";
  }
  ...
  public boolean isTop() {
    return true; // for refdes, value etc...
  }
